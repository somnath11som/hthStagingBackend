      ...
      for (i = 0; i < carModel.length; i++) {
        ...
        for (k = 0; k < travelInfo.length; k++) {
          ...
          if (placeName.length > 0) {
            ...
            LikeName = await PlacePrice.findOne({
              place: new RegExp(stateName),
            });
          ...
          let options = {
            ...
            message: `Dear Customer,<br>
                        ...
                        Booking Date: ${new Date(
                          bookingInsert.created_at ?? ""
                        )},<br>
                        Start date: ${new Date(
                          bookingInsert.bookingDate ?? ""
                        )},<br>
                        Pickup Time: ${bookingInsert.pickupTime ?? ""},<br>
                        Pickup Location: ${
                          bookingInsert.travelerInfo.pickupLocation ?? ""
                        },<br>
                        Total Days: ${
                          bookingInsert.totalDays ?? ""
                        },</b><br><br>
              ...
              bookingInsert.pnrno
            }">click here</a></b> open your vehicle verification QR code. You need to scan before trip start with driver for verify your trip. After verification your trip will be start. Wish you a happy journey. 
                        <br><br>Regards 
                        <br>HTH CABS support team`,
          };
          response(201, 1, bookingInsert, res);
          const updateBackup = await BookingBackup.updateOne(
            { pnrno: req.body.backupPnrno },
...
exports.getAllBooking = async (req, res, next) => {
  try {
    const allBooking = await Booking.find({ userId: req.authUser.id })
      .populate("car", "registration.CarNumber")
      .populate("driver");
    if (allBooking) {
...
exports.verifySignatureTest = async (req, res) => {
  if (
    ...
    let body = req.body.razorpay_order_id + "|" + req.body.razorpay_payment_id;
    let expectedSignature = crypto
      .createHmac("sha256", process.env.KEY_SECRET_TEST)
      .update(body.toString())
      .digest("hex");
    let response = { signatureIsValid: "false" };
    if (expectedSignature === req.body.razorpay_signature) {
...
exports.updateArriveStatus = async (req, res, next) => {
  try {
    ...
    if (update.n == 0) {
      return res
        .status(200)
        .json({ status: 0, msg: "Status not updated! Please try again." });
    } else {
      console.log(update.n > 0);
      return res
        .status(201)
        .json({ status: 1, msg: "Status update successfully" });
    }
  } catch (error) {
      ...
      if (days == totalDays) {
        ...
        if (bookingInsert) {
          return response(
            201,
            1,
            { backupPnrno: dbDate, msg: "Backup created successfully" },
            res
          );
        } else {
          return response(404, 0, "Something error! Please try again", res);
